...
#                 current["bm25_score"] = max(current.get("bm25_score", 0),chunk.get("bm25_score", 0),)
...
def merge_adjacent_chunks(chunks: list[dict]) -> list[dict]:
    with logfire.span("Post Processing - Merge Adjacent Chunks"):
        ...
        # Sort by document, page and chunk number
        chunks = sorted(
            chunks,
            key=lambda c: (
                c["document_id"],
        ...
        for chunk in chunks[1:]:
            ...
            if same_document and same_page and adjacent:
                ...
                # Preserve the best dense similarity score
                current["score"] = max(
                    current.get("score", 0),
                    chunk.get("score", 0),
                )

                # Preserve the best BM25 score
                current["bm25_score"] = max(
                    current.get("bm25_score", 0),
                    chunk.get("bm25_score", 0),
                )

                # Preserve the best RRF score
                current["rrf_score"] = max(
                    current.get("rrf_score", 0),
                    chunk.get("rrf_score", 0),
                )

                # Preserve the best reranker score
